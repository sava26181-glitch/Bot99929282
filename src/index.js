...
function startHealthServer() {
  const server = http.createServer((req, res) => {
    ...
    if (u.pathname.startsWith("/novnc/")) {
      // noVNC static files (HTML/JS/CSS/images) must be reachable without
      // the session token. The actual WebSocket connection below is protected
      // by SESSION_TOKEN. The previous version returned 401 for static assets,
      // which caused the red "Ошибка подключения к серверу" screen.
