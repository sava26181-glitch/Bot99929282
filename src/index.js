...
function startHealthServer() {
  const server = http.createServer((req, res) => {
    ...
    if (u.pathname.startsWith("/novnc/")) {
      // Static noVNC assets must be accessible without the session token; otherwise
      // vnc.html can load but its JS/CSS/images receive 401 responses. The actual
      // WebSocket endpoint below remains protected by SESSION_TOKEN.
...
for (const [action, handler] of [
  ["login", async ctx => {
    ...
    try {
      ...
      if (!host) throw new Error("Render не передал RENDER_EXTERNAL_HOSTNAME");
      const link = `https://${host}/browser?token=${encodeURIComponent(process.env.SESSION_TOKEN)}`;
      await ctx.reply("🔐 Открой браузер и войди в TikTok:", Markup.inlineKeyboard([[Markup.button.url("🌐 Войти в TikTok", link)]]));
      await ctx.reply("После входа просто вернись в Telegram. Сессия сохранится на сервере. Пароль сюда не отправляй.");
    } catch (e) {
      await ctx.reply("❌ " + e.message);
    }
  }],
