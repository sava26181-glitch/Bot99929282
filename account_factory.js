...
async function createMailTm() {
  ...
  // Mail.tm may return either a Hydra collection or a single domain object.
  const domains = Array.isArray(domainRes?.['hydra:member'])
    ? domainRes['hydra:member'].filter(x => x && typeof x.domain === 'string' && x.domain.trim())
    : (domainRes && typeof domainRes.domain === 'string' && domainRes.domain.trim()
      ? [domainRes]
      : []);
...
/* ============================================================
 *  СОЗДАНИЕ ОДНОГО АККАУНТА (параллельно-безопасное)
 * ============================================================ */
...
async function createOneAccount({ seq, nickname, niche, log = () => {} }) {
  ...
  try {
    // 1. Почта (параллельно с регистрацией device)
...
/* ============================================================
 *  ПАРАЛЛЕЛЬНАЯ ФАБРИКА С РОТАЦИЕЙ ПУЛА
 * ============================================================ */
